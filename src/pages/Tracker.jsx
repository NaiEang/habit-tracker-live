...
export default function Tracker() {
  ...
  const handleAddHabit = async (e) => {
    ...
    try {
      // First attempt standard title schema
      let payload = {
        title: titleText,
  ...
  const saveEdit = async (id) => {
    ...
    try {
      ...
      // Fallback if table uses 'name' column instead of 'title'
  ...
  return (
    <div style={{ maxWidth: 1000, margin: '0 auto', padding: '36px 20px', minHeight: '100vh' }}>
      {/* Top Navbar */}
      <header style={{
        display: 'flex',
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          ...
          <div>
            ...
            <p style={{ color: 'var(--text-muted)', fontSize: '0.85rem' }}>
              Live Supabase Database &bull; Protected by Row-Level Security
            </p>
          </div>
        </div>
      ...
      {showSessionDetails && (
        <section className="glass-panel" style={{ padding: 24, marginBottom: 28, borderColor: 'rgba(62, 207, 142, 0.3)' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 14 }}>
            ...
            <div>
              <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)', textTransform: 'uppercase' }}>RLS Scope</span>
              <div className="code-block" style={{ marginTop: 4, padding: '8px 10px', fontSize: '0.8rem', color: 'var(--brand-green)' }}>
                auth.uid() = {user?.id?.slice(0, 8)}...
              </div>
            </div>
          </div>
        </section>
      ...
      <section className="glass-panel" style={{ padding: '24px 28px' }}>
        ...
        {loading ? (
          ...
          /* ============================================================ */
          /* EMPTY LIST STATE (CRITICAL FOR SECOND ACCOUNT AUDIT CHECK)   */
          /* ============================================================ */
          <div style={{
            textAlign: 'center',
            padding: '50px 20px',
